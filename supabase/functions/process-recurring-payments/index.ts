...
serve(async req => {
  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return new Response(JSON.stringify({ error: '인증 헤더가 없거나 올바르지 않습니다' }), {
        status: 401,
    ...
    for (const sub of subscriptions as SubscriptionData[]) {
      try {
        ...
        if (paymentResult.success) {
          // 결제 성공 시 구독 기간 연장
          const interval =
            sub.plan_interval === 'monthly' ? 30 * 24 * 60 * 60 * 1000 : 365 * 24 * 60 * 60 * 1000;
    ...
    return new Response(JSON.stringify({ error: err.message || '알 수 없는 오류' }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
