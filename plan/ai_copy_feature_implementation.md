...
export default function AICopyForm({ onSubmit, isGenerating, copyType }: AICopyFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 p-4 bg-base-200 rounded-lg">
      <div className="mb-2">
        <label className="block text-sm font-medium mb-1">추가 요청사항이 있으신가요?</label>
        <textarea
          value={userPrompt}
    ...
    <div className="mt-6 p-4 border border-base-300 rounded-md bg-base-100 shadow-lg animate-fadeIn">
      {isSaved && (
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-semibold text-lg">{getCopyTypeLabel(copyType)} (저장됨)</h3>
          <button onClick={onNewCopy} className="btn btn-sm btn-outline">
            새로 만들기
          </button>
        </div>
      )}
...
export default function PlaceDetailPage(props: { params: { placeId: string } }) {
  // ... 기존 상태 ...
