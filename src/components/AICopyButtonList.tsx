      ...
      {items.map(item => {
        ...
        return (
          <button
            ...
            className={`px-4 py-1.5 btn-primary rounded-md border text-md font-medium transition-all border-2 border-primary cursor-pointer
              ${
                isActive
          ...
          >
            <span className="flex items-center gap-1">
              ...
              {isSaved && (
                <span
                  className={`w-2 h-2 rounded-full inline-block ${isActive ? 'bg-primary' : 'bg-white'}`}
                ></span>
              )}
            </span>
          </button>
