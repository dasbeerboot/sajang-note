...
export async function POST(request: Request) {
  try {
    ...
    // 회원가입이 아닐 때만 중복 확인 (회원가입은 이미 check-phone API에서 확인됨)
    if (!isSignup) {
      ...
      // 다른 사용자가 인증한 번호라면 에러
      if (userId && existingPhone && existingPhone.id !== userId) {
        return NextResponse.json(
    ...
    if (recentCodes && recentCodes.length >= 3) {
      return NextResponse.json(
        { error: '너무 많은 인증번호 요청이 있었습니다. 잠시 후 다시 시도해주세요.' },
        { status: 429 }
      );
    }
    ...
    // 새 인증번호 저장
    const { error: insertError } = await serviceClient.from('verification_codes').insert({
      phone,
      code,
