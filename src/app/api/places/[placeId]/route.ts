...
// 매장 상세 정보 조회 API
export async function GET(req: NextRequest, { params }: { params: { placeId: string } }) {
  try {
    ...
    if (!placeId) {
      return NextResponse.json({ error: '매장 ID가 제공되지 않았습니다.' }, { status: 400 });
    }
    ...
    if (place.user_id !== user.id) {
      return NextResponse.json({ error: '이 매장에 접근할 권한이 없습니다.' }, { status: 403 });
    }
...
export async function DELETE(req: NextRequest, { params }: { params: { placeId: string } }) {
  try {
    const cookieStore = await cookies();
    ...
    // 소유권 확인
    if (place.user_id !== user.id) {
      return NextResponse.json({ error: '이 매장에 대한 삭제 권한이 없습니다.' }, { status: 403 });
    }
    ...
    const nextChangeDate = profile.next_place_change_date
      ? new Date(profile.next_place_change_date)
      : null;
    const now = new Date();
    ...
    if (deleteAiCopiesError) {
      ...
      return NextResponse.json({ error: 'AI 카피 삭제 중 오류가 발생했습니다.' }, { status: 500 });
    }
