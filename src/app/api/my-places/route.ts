...
export async function GET() {
  try {
    ...
    const { data: profileData, error: profileError } = await serviceClient
      .from('profiles')
      .select(
        'max_places, next_place_change_date, first_place_change_used, subscription_tier, remaining_place_changes'
      )
      .eq('id', userId)
      .single();
    ...
    // 변경 가능 여부 확인
    const { data: changeInfo, error: changeError } = await serviceClient.rpc(
      'get_user_change_info',
      {
        p_user_id: userId,
      }
    );
    ...
    if (changeError) {
      ...
      const defaultChangeInfo = {
        can_change_place:
          profileData.remaining_place_changes > 0 || !profileData.first_place_change_used,
    ...
    return NextResponse.json(
      { error: '요청 처리 중 오류가 발생했습니다.', details: errorMsg },
      { status: 500 }
    );
  }
}
