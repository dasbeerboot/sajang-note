...
export default function ProfilePage() {
    ...
    async function loadProfileData() {
      ...
      try {
        ...
        const { data, error } = await supabase
          .from('profiles')
          .select(
            'id, full_name, phone, phone_verified, subscription_status, subscription_end_date, billing_id, subscription_tier'
          )
          .eq('id', user.id)
          .single();
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : '구독 취소 중 알 수 없는 오류가 발생했습니다.';
          ...
          {profileData?.subscription_status === 'active' ? (
            <div className="py-2">
              <div className="p-6 border border-success rounded-lg shadow-sm bg-success/5">
                ...
                <p className="text-xs text-base-content opacity-60">
                  구독을 취소하면 다음 결제일부터 자동 결제가 중단되며, 현재 구독 기간 만료일까지는
                  계속 서비스를 이용할 수 있습니다.
                </p>
              </div>
            </div>
              ...
              <div className="p-6 border border-warning rounded-lg shadow-sm bg-warning/5">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                  <div className="mb-4 sm:mb-0">
                    ...
                    {profileData.subscription_end_date && (
                      <p className="text-sm text-base-content opacity-80">
                        {profileData.subscription_tier === 'premium' ? 'Pro' : 'Basic'} 플랜 이용
                        기간:{' '}
                        {new Date(profileData.subscription_end_date).toISOString().split('T')[0]}
                        까지
                      </p>
            ...
            <div className="py-2">
              <div className="text-center p-6 border border-dashed border-base-300 rounded-lg">
                ...
                <button onClick={handleSubscribe} className="btn btn-primary btn-md">
                  구독 시작하기
                </button>
              </div>
            </div>
