...
export default function SignupPage() {
  ...
  const handleSendVerification = async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : '인증번호 발송 중 알 수 없는 오류가 발생했습니다.';
      setError(message);
    } finally {
  ...
  const handleFinalSignup = useCallback(async () => {
    console.log(
      '[handleFinalSignup] Called. phoneVerified:',
      phoneVerified,
      'isAttemptingFinalSignup:',
      isAttemptingFinalSignup
    );
    ...
    console.log('[handleFinalSignup] Proceeding with signup. User details:', {
      name,
      email,
      phone_raw: phone,
    });
    ...
    try {
      ...
      const { data: signUpResponse, error: signUpError } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            // 이 정보가 NEW.raw_user_meta_data로 전달되어 트리거가 사용합니다.
            full_name: name,
            phone: cleanedPhone,
            phone_verified: true, // 전화번호 인증 완료
            user_status: 'pending', // 이메일 인증 대기 상태
            is_returning_user: localStorage.getItem('isReturningUser') === 'true', // 이전 가입이력 상태 추가
          },
      ...
      console.log(
        '[handleFinalSignup] supabase.auth.signUp successful for user:',
      ...
      console.log(
        '[handleFinalSignup] Finally block. Setting loading and isAttemptingFinalSignup to false.'
      );
      setLoading(false);
      setIsAttemptingFinalSignup(false);
    }
  ...
  const handleVerifyCodeAndProceed = async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : '인증번호 확인 중 알 수 없는 오류가 발생했습니다.';
      setError(message);
    } finally {
      setIsVerifyingPhone(false);
    }
  ...
  useEffect(() => {
    console.log(
      '[useEffect_FinalSignup] Detected state change. phoneVerified:',
      phoneVerified,
      'isAttemptingFinalSignup:',
      isAttemptingFinalSignup
    );
  ...
  const handlePrimarySubmit = async (e: React.FormEvent) => {
    ...
    if (step === 1) {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(data.error || '전화번호 확인 중 오류가 발생했습니다.');
        }

        // 이전에 사용한 이력이 있는 경우 알림
        if (data.isReturningUser) {
          localStorage.setItem('isReturningUser', 'true');
          showToast('이전에 사용한 번호입니다. 무료 체험 횟수는 0회로 설정됩니다.', 'warning');
        } else {
          localStorage.setItem('isReturningUser', 'false');
        }

        // 중복이 아닌 경우에만 다음 단계로 진행
        setStep(2); // 전화번호 인증 단계로
        handleSendVerification(); // 인증번호 발송
      } catch (error: unknown) {
        const message =
          error instanceof Error
            ? error.message
            : '전화번호 확인 중 알 수 없는 오류가 발생했습니다.';
        setError(message);
        setIsVerifyingPhone(false);
      }
      ...
      <div className="w-full max-w-md space-y-6 bg-base-200 p-8 rounded-xl shadow-xl">
        ...
        {step === 1 && (
          <form onSubmit={handlePrimarySubmit} className="space-y-4">
            ...
            <div className="text-center text-sm">
              이미 계정이 있으신가요?{' '}
              <Link href="/?openLoginModal=true" className="link link-primary">
                로그인
              </Link>
            </div>
          </form>
          ...
          <form onSubmit={handlePrimarySubmit} className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={isVerifyingPhone || verificationCode.length !== 6 || phoneVerified}
              >
        ...
        {step === 3 && (
          <div className="text-center space-y-4 py-4">
            ...
            <p className="text-xs opacity-70">이메일을 받지 못하셨다면 스팸함도 확인해주세요.</p>
